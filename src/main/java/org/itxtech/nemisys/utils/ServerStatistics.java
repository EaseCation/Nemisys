...
public final class ServerStatistics implements DynamicMBean {
    ...
    private ServerStatistics(Server server) {
        ...
        this.server = server;
        this.mBeanInfo = new MBeanInfo("NemisysServerStatistics", "metrics for Nemisys server",
    ...
    public static void registerJmxMonitoring(Server server) {
        try {
            ManagementFactory.getPlatformMBeanServer().registerMBean(new ServerStatistics(server), new ObjectName("org.itxtech.nemisys:type=Server"));
