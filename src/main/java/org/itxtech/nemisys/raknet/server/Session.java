    ...
    private void handleEncapsulatedPacketRoute(EncapsulatedPacket packet) throws Exception {
        ...
        if ((id & 0xff) < 0x80) { //internal data packet
            if (state == STATE_CONNECTING_2) {
                ...
                //游戏时钟加速挂检测 如果发现客户端发送PING_PACKET过快，则传输一个负延迟到应用层
                //TODO 会拉低性能，考虑在其他线程计算？
                if (recordClientPingQueue) {
                    this.clientPingQueue.offer(System.currentTimeMillis());
                    if (this.clientPingQueue.size() > 10) this.clientPingQueue.poll();
                    double[] pingInterval = new double[this.clientPingQueue.size() - 1];
                    Long[] pings = this.clientPingQueue.toArray(new Long[0]);
                    for (int i = 0; i < pings.length - 1; i++) pingInterval[i] = pings[i+1] - pings[i];
                    double ave = MathUtil.getAverage(pingInterval);
                    //double variance = MathUtil.getStandardDiviation(pingInterval);
                    //Server.getInstance().getLogger().warning("PING: ave=" + ave + " variance=" + variance);

                    if (this.clientPingQueue.size() >= 10 && ave < 2200) {
                        //检测到不正常的PING包速率，返回PING=-1给应用层
                ...
                if (state == STATE_CONNECTED) {
                    ...
                    if (state == STATE_CONNECTED) {
                        PING_DataPacket pingPacket = new PING_DataPacket();
                        pingPacket.pingID = System.currentTimeMillis() - dataPacket.pingID;
                        pingPacket.encode();
                        packet.buffer = pingPacket.buffer;
                        this.sessionManager.streamEncapsulated(this, packet);
                    }
