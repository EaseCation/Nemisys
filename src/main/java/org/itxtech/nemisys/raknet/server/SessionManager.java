...
public class SessionManager {
    ...
    private void tickProcessor() throws Exception {
        ...
        while (!this.shutdown) {
            ...
            if (time > 10) {
                Server.getInstance().getLogger().alert("RakNet线程已过载！1Tick用时 " + time + "ms");
            } else if (time > 8) {
                Server.getInstance().getLogger().warning("RakNet线程已过载！1Tick用时 " + time + "ms");
            }
            if (time < 10) {
                try {
	...
	private void log(byte[] buffer) {
		StringBuilder hexLog = new StringBuilder("in : ");
		if(buffer[0] != (byte) 0x01) { // 不打印Unconnected PING包，太多了
			hexLog.append(Hex.bytesToHex(buffer));
			this.getLogger().debug(hexLog.toString());
		}
	}
