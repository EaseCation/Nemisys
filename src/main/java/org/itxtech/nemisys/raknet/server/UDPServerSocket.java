...
public class UDPServerSocket extends ChannelInboundHandlerAdapter {
    ...
    public int writePacket(byte[] data, InetSocketAddress dest) throws IOException {
    	StringBuilder hexLog = new StringBuilder("out : ");
    	if(data[0] != (byte)0x1c) { // 不记录unconnected pong 的包，太多了
    		hexLog.append(Hex.bytesToHex(data));
    		this.logger.debug(hexLog.toString());
    	}
